...
function updateCanvas() {
    // limpar o desenho (como fizemos em aula)
    // Sortear um novo índice para selecionar um novo item do quickDrawDataset (parecido com o que foi feito nas linhas anteriores)
    // atualizar a variável que contém o item da array (parecido com o que foi feito nas linhas anteriores)
    // mostrar o novo item a ser desenhado na tela (parecido com o que foi feito nas linhas anteriores)
}
...
function checkSketch() {
    // aumentar o valor da variável timerCounter em 1 e mostrar o resultado na tela (esta função é constantemente chamada no function draw())
    //verifica se timerCounter já chegou no tempo pré-estabelecido
    if (timerCounter > 400) {
        // zerar o timerCounter (já que o tempo já chegou no máximo)
        // alterar o timerCheck para "completed" para informar ao computador que já podemos ir para o próximo esboço
    }
    //verifica se o tempo acabou OU se a pessoa respondeu certo
    if (timerCheck == "completed" || answerHolder == "set") {
        // definir timerCheck e answerHolder como textos vaxios novamente
        // chamar a função updateCanvas() para que um novo ciclo de adivinhaçoes comece
    }
